...
var userId = user[0].id;
	$(function() {
		$('#rbac-user-role-dg').datagrid({
			url:'${ctx}/manager/allRoles.do',
			queryParams: {
				userId:userId,
				_: getRandomTime
			},
			...
			columns:[[
				...
				if(data){
			            $.each(data.rows, function(index, item){
			               if(item.checked){
			                  $('#rbac-user-role-dg').datagrid('checkRow', index);
			                }
			            });
			        }	
			}
...
<div class="easyui-layout" data-options="fit:true">
	<div region="north" border="false" class="easyui-panel" style="height: 40px; padding: 5px;">
		<form id="rbac-user-editRole-form" action="#">
			当前用户：<input type="text" name="username" readonly/>
			<a id="rbac-user-editRole-save-button" href="javascript:void(0)" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-save'">保存角色</a>
		</form>
	</div>
	
	<div region="center" border="false">
	    <table id="rbac-user-role-dg"></table>
	</div>
</body>
</html>
