...
var roleId = role[0].id;
	$(function() {
		$('#rbac-role-permission-dg').datagrid({
			url:'${ctx}/manager/allPermissions.do',
			queryParams: {
				roleId:roleId,
				_: getRandomTime
			},
			...
			columns:[[
				...
				if(data){
			            $.each(data.rows, function(index, item){
			               if(item.checked){
			                  $('#rbac-role-permission-dg').datagrid('checkRow', index);
			                }
			            });
			        }	
			}
...
<div class="easyui-layout" data-options="fit:true">
	<div region="north" border="false" class="easyui-panel" style="height: 40px; padding: 5px;">
		<form id="rbac-role-editPermission-form" action="#">
			当前角色：<input type="text" name="role" readonly/>
			<a id="rbac-role-editPermission-save-button" href="javascript:void(0)" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-save'">保存权限</a>
		</form>
	</div>
	
	<div region="center" border="false">
	    <table id="rbac-role-permission-dg"></table>
	</div>
</body>
</html>
